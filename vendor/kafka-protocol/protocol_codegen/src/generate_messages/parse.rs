...
pub fn parse(path: &Path) -> Result<Spec, Error> {
    ...
    convert_string_booleans(&mut original_json);
...
fn convert_string_booleans(value: &mut Value) {
    match value {
        Value::Object(object) => {
            for (key, value) in object {
                if matches!(
                    key.as_str(),
                    "ignorable" | "latestVersionUnstable" | "mapKey" | "zeroCopy"
                ) {
                    if let Some(parsed) = value.as_str().and_then(|value| value.parse().ok()) {
